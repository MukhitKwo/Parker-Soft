...
void Play() {
  ...
  for (int b = 0; b < times * 2; b++) {
    int x = (b % 2 == 0) ? 0 : -updates;
    int y = x + updates;
    ...
    for (int c = x; c < y; c++) {
      ...
      pca1.setPWM(1 * 3 + 1, 0, 115 + (180 - (((55.0 - 15.0) / updates) * c + 15)) * 2.694);
      pca1.setPWM(1 * 3 + 2, 0, 115 + (180 - (((120.0 - 170.0) / updates) * c + 170)) * 2.694);
      delay(10);
    }
  }
}
...
void Turn() {
  if (command[1] == "off") {
    ...
    if (command[2] == "0") {  //set position zero before turning OFF
      Set(true, 0);
      delay(600);
    }
