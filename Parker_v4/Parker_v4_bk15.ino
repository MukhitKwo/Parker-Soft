...
void Play() {
  ...
  if (command[1] == "step" || command[1] == "f") {  //tecnically stepping, but almost the same thing
    Step(times);
  } else if (command[1] == "jump" || command[1] == "b") {  //not jump, but bouncing the robot
    Jump(times);
  } else if (command[1] == "spin" || command[1] == "l") {  //not spin, but oscillate the robot
    Spin(times);
  } else if (command[1] == "wave" || command[1] == "r") {  //bonce left and right, forgot name
    if (command[3] == "fix") {
      Set(true, 20);
    } else {
      Set(true, 2);
...
void Jump(int8_t times) {
  ...
  for (int z = 0; z < times; z++) {
    ...
    int degC1 = (sense == 1) ? 50 : 90;
    int degC2 = (sense == 1) ? 90 : 50;

    int degT1 = (sense == 1) ? 6 : 28;
...
void Set(bool called, int8_t movement) {
  ...
  if (movement == 0) {  //set the robot to its zero position (all zero)
    ...
    Move();
  } else if (movement == 1 || movement == 10) {  //set the robot to its normal position (all one)

    if ((command[2] == "fix" && !called) || movement == 10) {

      for (int k = 0; k < 2; k++) {
        splitStringIntoWords("move " + String((k == 0) ? 16 : 34) + " 110 90 50");
        Move();
        delay(500);
        pca1.setPWM(((k == 0) ? 0 : 2) * 3 + 1, 0, 115 + (180 - 70) * 2.694);
        pca2.setPWM(((k == 0) ? 2 : 0) * 3 + 1, 0, 115 + (180 - 70) * 2.694);
        delay(500);
      }
    }
  ...
  } else if (movement == 2 || movement == 20) {  //set the robot to its walk/rotate postition

    if ((command[2] == "fix" && !called) || movement == 20) {

      for (int k = 0; k < 2; k++) {
        splitStringIntoWords("move " + String((k == 0) ? 16 : 34) + " 110 " + String((k == 0) ? 65 : 115) + " 50");
  ...
  } else if (movement == 3) {  //mix betwen zero postion and walk/rotate position
    Set(true, 2);
